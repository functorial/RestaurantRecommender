...
class LRFinder(object):
    ...
    def _train_batch(self, train_iter, accumulation_steps, non_blocking_transfer=True):
        ...
        for i in range(accumulation_steps):
            ...
            neg_v_id = get_negative_samples(low_incl=1, high_excl=101, seqs=inputs)
            outputs = self.model.forward(inputs, labels)
            neg_pred = self.model.forward(inputs, neg_v_id)
            mr_loss_target = torch.ones(outputs.size())
            loss = self.criterion(outputs, neg_pred, mr_loss_target)
